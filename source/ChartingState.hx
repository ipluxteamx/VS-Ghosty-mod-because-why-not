...
class ChartingState extends MusicBeatState
{
	...
	var noteKind:Int = 0;
	var kinds:Array<String> = ["normal", "flip", "hurt"];
	...
	**/
	...
	override function update(elapsed:Float)
	{
		...
		if (!typingShit.hasFocus)
		{
			...
			if (FlxG.keys.justPressed.O) {
				this.noteKind --;
				if (noteKind < 0) {
					noteKind = kinds.length - 1;
				}
			}

			if (FlxG.keys.justPressed.P) {
				this.noteKind ++;
				if (noteKind == kinds.length) {
					noteKind = 0;
				}
			}
	...
	function updateGrid():Void
	{
		...
		for (section in _song.notes)
		{
			for (i in section.sectionNotes)
			{
				...
				var note:Note = new Note(daStrumTime, daNoteInfo % 4,null,false,true,daKind,i[3], i[4]);
	...
	private function addNote(?n:Note):Void
	{
		...
		var noteSus = 0;
		var noteKind = kinds[this.noteKind];

		if (n != null)
			section.sectionNotes.push([n.strumTime, n.noteData, n.sustainLength, n.noteKind, false, TimingStruct.getBeatFromTime(n.strumTime)]);
		else
			section.sectionNotes.push([noteStrum, noteData, noteSus, noteKind, false, TimingStruct.getBeatFromTime(noteStrum)]);
